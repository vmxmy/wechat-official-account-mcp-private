...
const qrcodeToolSchema = z.object({
  ...
  sceneId: z.number().int().positive().optional().describe('场景值ID（临时二维码:32位非0整数, 永久二维码:1-100000'),
  sceneStr: z.string().optional().describe('场景值字符串（仅永久二维码支持）'),
  expireSeconds: z.number().int().positive().max(2592000).optional().describe('临时二维码有效期（秒），最大30天（2592000）'),
...
export const qrcodeMcpTool: McpTool = {
  ...
  inputSchema: {
    ...
    sceneId: z.number().int().positive().optional().describe('场景值ID（临时二维码:32位非0整数, 永久二维码:1-100000）'),
    sceneStr: z.string().optional().describe('场景值字符串（仅永久二维码支持）'),
    expireSeconds: z.number().int().positive().max(2592000).optional().describe('临时二维码有效期（秒），最大30天'),
    ticket: z.string().optional().describe('二维码ticket，用于换取二维码图片URL'),
  },
  handler: async (params: unknown, apiClient: WechatApiClient) => {
    ...
    if (args.action === 'create_temp') {
      if (args.sceneId === undefined && !args.sceneStr) {
    ...
    if (args.action === 'create_permanent') {
      ...
      if (args.sceneId !== undefined && args.sceneId > 100000) {
        throw new Error('永久二维码 sceneId 官方限制为 1-100000');
      }
