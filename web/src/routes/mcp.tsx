  ...
  return (
    <>
      <PageHeader
        eyebrow="远程连接"
        title="远程 MCP 配置"
        description="复制远程 MCP 地址或客户端配置，并在 Codex、Claude Code 中完成 OAuth 登录。"
      />
      <PageStack>
        <SurfaceSection title="远程 MCP 地址" tone="accent">
          <VStack gap={2}>
            <p className="section-copy mono">{mcpUrl()}</p>
            <p className="section-copy">授权由客户端通过 OAuth 完成；不要把 AppSecret 或访问令牌写进 MCP 配置文件。</p>
          </VStack>
        </SurfaceSection>
        <SurfaceSection title="客户端配置">
          <PageGrid columns={{ minWidth: 260, max: 2 }}>
            <VStack gap={2}>
