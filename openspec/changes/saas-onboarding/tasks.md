...
- [ ] 1.1 Run `openspec validate saas-onboarding` and fix schema/format issues before implementation.
...
- [ ] 2.1 Add additive D1 migration for Operators, identities, email codes, Web sessions, OAuth clients, OAuth consents, OAuth refresh/session records, and public signup rate-limit counters.
- [ ] 2.2 Add or reconcile D1 schema for Tenants, tenant owners, WeChat resources, default resource selection, encrypted credential fields, resource deletion status, and active AppID uniqueness.
...
- [ ] 2.4 Add D1 schema for audit logs, monitoring events, R2 media retention metadata, inbound message retention metadata, and operator deletion requests.
- [ ] 2.5 Implement repository methods for Operator identity create/link/lookup by verified email and provider subject.
- [ ] 2.6 Implement repository methods for email-code issue/verify/expire/attempt-count and signup/login rate limits.
- [ ] 2.7 Implement repository methods for Web sessions, OAuth clients, consent records, token/session revocation, and refresh token expiry.
- [ ] 2.8 Implement repository methods for Tenant bootstrap, owner lookup, WeChat resource create/list/get/rename/default/delete, AppID uniqueness, and encrypted credential save/clear.
- [ ] 2.9 Implement repository methods for subscription entitlements, Stripe mapping, usage period resolution, quota reservation, and over-quota error details.
- [ ] 2.10 Add repository tests for cross-tenant denial, duplicate AppID denial, deleted AppID release, failed credential non-persistence, quota period math, and secret redaction.
...
- [ ] 4.2 Implement opaque ID generation for Tenants and WeChat resources and migrate code away from relying on `tenant_default/acct_default` as public IDs for new tenants.
- [ ] 4.3 Implement WeChat resource creation subject to Free/Plus/Pro account allowance.
...
- [ ] 4.6 Ensure failed credential validation does not persist AppSecret and returns actionable relay/IP allowlist guidance.
...
- [ ] 5.3 Implement Web/CLI checkout session creation for Plus/Pro and reject MCP checkout creation.
...
- [ ] 5.8 Implement tool-call allowances Free=300, Plus=3000, Pro=30000 across Web/CLI/MCP/API protected operations.
...
- [ ] 6.1 Add `@astryxdesign/core`, `@astryxdesign/cli`, TanStack Router, TanStack Router Vite plugin, and TanStack Query dependencies; remove React Router once TanStack routes are wired.
- [ ] 6.2 Move Web source to `web/`, configure Vite/TanStack file routes, output `web/dist`, and update Worker static asset serving and build scripts.
- [ ] 6.3 Run Astryx dense docs bootstrap and record component/template choices before implementing Web pages.
- [ ] 6.4 Implement Astryx theme CSS with required layer order, `ThemeProvider`, and `LinkProvider` with a central TanStack `AppLink` adapter.
...
- [ ] 6.6 Implement TanStack Query provider, query keys, mutation invalidation, and route guards with `returnTo` redirect behavior.
...
- [ ] 6.9 Implement billing page and Stripe success/cancel/status routes.
- [ ] 6.10 Implement MCP config page that outputs native Streamable HTTP config for Codex/Claude without OAuth tokens.
- [ ] 6.11 Implement security sessions page for authorized client/session listing and revocation.
- [ ] 6.12 Implement Terms and Privacy pages with `support@ziikoo.app` contact and credential/payment/retention disclosures.
...
- [ ] 7.1 Rename public package metadata to `@ziikoo/woa` while preserving `woa` bin and private repository metadata policy.
...
- [ ] 7.3 Implement CLI commands for `whoami`, account list/create/rename/default/configure/status/delete, quota/status, and Tenant context display.
- [ ] 7.4 Implement `woa billing checkout --plan plus|pro` to create checkout, print URL, and open browser.
- [ ] 7.5 Implement MCP config generation for Codex and Claude pointing to `https://woa.ziikoo.app/mcp` without tokens or local MCP transports.
- [ ] 7.6 Enforce destructive delete confirmation before remote delete calls.
- [ ] 7.7 Add CLI smoke tests with mocked server responses proving no WeChat secrets are stored locally and no local MCP/stdio/SSE commands are restored.
...
- [ ] 8.4 Remove or reject Tenant create from MCP in the first release with clear guidance.
...
- [ ] 8.6 Ensure MCP tools remain visible for Free Tenants while quota enforcement protects operations.
...
- [ ] 10.1 Update README with hosted SaaS onboarding, email-first login, Web/CLI/MCP entrypoints, native Streamable HTTP MCP config, plan limits, and platform relay allowlist instructions.
- [ ] 10.2 Update CLI help text and examples for `@ziikoo/woa`, email-first login, billing checkout, account configuration, and MCP config generation.
- [ ] 10.3 Document legal/support pages and `support@ziikoo.app` contact.
- [ ] 10.4 Document video publishing as unsupported in the first release while article and image/贴图 publishing are supported.
- [ ] 10.5 Document migration behavior: legacy password removal, legacy secret purge, required reconfiguration, and accepted short downtime.
- [ ] 10.6 Run Chinese copy anti-slop review for Web, CLI, README, and onboarding error messages.

## 11. Verification and release gates

- [ ] 11.1 Run `openspec validate saas-onboarding`.
- [ ] 11.2 Run `npm run check`.
- [ ] 11.3 Run `npm run lint`.
- [ ] 11.4 Run `npm test`.
- [ ] 11.5 Run Web build and Worker dry-run deploy validation.
